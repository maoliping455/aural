...
if dummy_model.kwargs["chunk_duration"] is not None:
    raise SystemExit(f"segmented ASR first pass should not request internal chunks: {dummy_model.kwargs}")
if dummy_model.kwargs["repetition_penalty"] != 1.0:
    raise SystemExit(f"segmented ASR should request neutral first-pass repetition penalty: {dummy_model.kwargs}")
...
retry_text, retry_event = worker.generate_text_with_repetition_retry(
    repeating_model,
    Path("sample.wav"),
    "zh",
...
if len(repeating_model.kwargs) != 2:
    raise SystemExit(f"segmented ASR should retry abnormal repetition once: {repeating_model.kwargs}")
...
if not retry_event or not retry_event["accepted_retry"] or retry_text.startswith("嗯"):
    raise SystemExit(f"segmented ASR should accept non-repeating retry text: {retry_event}, {retry_text}")
