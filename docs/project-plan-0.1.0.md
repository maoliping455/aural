...
| --- | --- | --- | --- | --- |
| A-001 | 配置 release 机器 Developer ID signing 和 notarytool keychain profile | Ready / Notarization In Progress | Release | release 机器已确认有效 `Developer ID Application` identity，`AuralNotaryProfile` 可查询 submission；当前等待 signed DMG notarization 完成后执行 staple 和 Gatekeeper 验证 |
