...
| P0-6 | ITN FST 发布策略 | 需要 PM 决策 | PM + Dev | 决策后跑 `validate-itn-postprocess.py` 或记录非阻塞 fallback |
| P0-7 | ASR 质量发布底线 | raw ASR 当前 blocker 已按默认策略回归关闭；规则继续阻塞新增 hard bad | PM + QA + Dev | 根因文档、默认参数修复、18-case bad-case 回归和真实模型 smoke |
...
| --- | --- | --- | --- | --- |
| A-001 | 配置 release 机器 Developer ID signing 和 notarytool keychain profile | Ready for Release Validation | Release | release 机器已确认 1 个有效 `Developer ID Application` identity；`xcrun notarytool history --keychain-profile AuralNotaryProfile` 已认证通过；随后可跑 signed/notarized DMG 流程 |
...
| ITN FST | PM 决定是否阻塞 | 待确认 | 当前 QA 记录显示 FST 缺失会导致 ITN 验证失败 |
